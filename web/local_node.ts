...
export function formatLocalNodeSessionOutput(session: AutonomousSession, options?: { json?: boolean; verbose?: boolean }): string {
  ...
  const lines = [
    ...
    ...(session.oversight.operatorAttention.length
      ? session.oversight.operatorAttention.map((item) => `- ${item.kind}: ${item.summary} Next action: ${item.recommendedOperatorAction}`)
      : ["- No operator attention is currently required."]),
    "",
    "Operator controls:",
    ...
    "",
    "Per-task review:",
    ...(session.oversight.taskReviews.length
      ? session.oversight.taskReviews.flatMap((review) => [
          `- ${review.taskId} | ${review.status} | ${review.title}`,
          `  selected=${review.whySelected}`,
          `  validation=${review.validationResult}`,
          `  repoActions=${review.repoActionsGenerated ? "yes" : "no"} | approvalNeeded=${review.approvalNeeded ? "yes" : "no"}`,
          `  outcome=${review.transitionSummary}`,
          `  outputs=${review.outputsProduced.join(" | ")}`,
        ])
      : ["- No per-task review records were generated."]),
    "",
    ...
    `Current deliverable target: ${session.workflowContinuity.coding.currentDeliverableTarget ?? "No deliverable target recorded."}`,
    `Expected output form: ${session.workflowContinuity.coding.expectedOutputForm ?? "No expected output form recorded."}`,
    `Validation success target: ${session.workflowContinuity.coding.validationSuccessTarget ?? "No validation success target recorded."}`,
    `Current acceptance target: ${session.workflowContinuity.coding.currentAcceptanceTarget ?? "No acceptance target recorded."}`,
    `Current target status: ${session.workflowContinuity.coding.currentTargetStatus}`,
    `Validation proves: ${session.workflowContinuity.coding.validationProves ?? "No validation proof target recorded."}`,
    `Validation target matches deliverable: ${String(session.workflowContinuity.coding.validationTargetMatchesDeliverable)}`,
    `Validation failure impact: ${session.workflowContinuity.coding.validationFailureImpact ?? "No deliverable impact recorded."}`,
    `Correction maintains deliverable: ${String(session.workflowContinuity.coding.correctionMaintainsDeliverable)}`,
    `Deliverable changed during correction or escalation: ${String(session.workflowContinuity.coding.deliverableChangedDuringCorrectionOrEscalation)}`,
    `Deliverable accepted: ${String(session.workflowContinuity.coding.deliverableAccepted)}`,
    `Acceptance confidence: ${session.workflowContinuity.coding.acceptanceConfidence}`,
    `Completion state: ${session.workflowContinuity.coding.completionState}`,
    `Operator confirmation required: ${String(session.workflowContinuity.coding.operatorConfirmationRequired)}`,
    `Loop should terminate: ${String(session.workflowContinuity.coding.shouldTerminateLoop)}`,
    `Acceptance reason: ${session.workflowContinuity.coding.acceptanceReason ?? "No acceptance reason recorded."}`,
    `Acceptance summary: ${session.workflowContinuity.coding.acceptanceSummary ?? "No acceptance summary recorded."}`,
    `Output linked to deliverable: ${String(session.workflowContinuity.coding.outputLinkedToDeliverable)}`,
    `Files affected: ${session.workflowContinuity.coding.outputArtifacts.length > 0 ? [...new Set(session.workflowContinuity.coding.outputArtifacts.map((artifact) => artifact.filePath))].join(", ") : "No output artifacts recorded."}`,
    `Last output summary: ${session.workflowContinuity.coding.lastOutputSummary ?? "No output summary recorded."}`,
    `Output artifacts: ${session.workflowContinuity.coding.outputArtifacts.length > 0 ? session.workflowContinuity.coding.outputArtifacts.map((artifact) => `step ${artifact.stepIndex}: ${artifact.filePath}${artifact.diffLikeSummary ? ` | diff=${artifact.diffLikeSummary}` : ""}${artifact.linkedToDeliverable ? " | linked=true" : " | linked=false"}`).join(" ; ") : "No output artifacts recorded."}`,
    `Approval state: ${session.workflowContinuity.coding.approvalStateSummary ?? "No approval state recorded."}`,
    `Repo action summary: ${session.workflowContinuity.coding.repoActionSummary ?? "No repo actions derived."}`,
    `Readiness guardrails: ${session.workflowContinuity.coding.integritySummary ?? "No readiness guardrails are currently active."}`,
    `Pending repo actions: ${session.workflowContinuity.coding.pendingRepoActions.length > 0 ? session.workflowContinuity.coding.pendingRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "No pending repo actions."}`,
    `Approved repo actions: ${session.workflowContinuity.coding.approvedRepoActions.length > 0 ? session.workflowContinuity.coding.approvedRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "No approved repo actions."}`,
    `Executed repo actions: ${session.workflowContinuity.coding.executedRepoActions.length > 0 ? session.workflowContinuity.coding.executedRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "No executed repo actions."}`,
    `Validation target: ${session.workflowContinuity.coding.validationTarget ?? "No validation target recorded."}`,
    `Current validation target: ${session.workflowContinuity.coding.currentValidationTarget ?? "No current validation target recorded."}`,
    `Last code change summary: ${session.workflowContinuity.coding.lastCodeChangeSummary ?? "No code change summary recorded."}`,
    `Last implementation summary: ${session.workflowContinuity.coding.lastImplementationSummary ?? "No implementation summary recorded."}`,
    `Last validation summary: ${session.workflowContinuity.coding.lastValidationSummary ?? "No validation summary recorded."}`,
    ...
    `Last correction summary: ${session.workflowContinuity.coding.lastCorrectionSummary ?? "No correction summary recorded."}`,
    ...
    `Validation-first coding behavior: ${String(session.workflowContinuity.coding.validationFirstActive)}`,
...
export function formatLocalNodeQueueRunOutput(summary: QueueExecutionSummary, options?: { json?: boolean }): string {
  ...
  return [
    ...
    `Current deliverable target: ${summary.session?.workflowContinuity?.coding?.currentDeliverableTarget ?? "unknown"}`,
    `Expected output form: ${summary.session?.workflowContinuity?.coding?.expectedOutputForm ?? "unknown"}`,
    `Validation success target: ${summary.session?.workflowContinuity?.coding?.validationSuccessTarget ?? "unknown"}`,
    `Current acceptance target: ${summary.session?.workflowContinuity?.coding?.currentAcceptanceTarget ?? "unknown"}`,
    `Current target status: ${summary.session?.workflowContinuity?.coding?.currentTargetStatus ?? "unknown"}`,
    `Validation proves: ${summary.session?.workflowContinuity?.coding?.validationProves ?? "unknown"}`,
    `Validation target matches deliverable: ${typeof summary.session?.workflowContinuity?.coding?.validationTargetMatchesDeliverable === "boolean" ? String(summary.session.workflowContinuity.coding.validationTargetMatchesDeliverable) : "unknown"}`,
    `Validation failure impact: ${summary.session?.workflowContinuity?.coding?.validationFailureImpact ?? "unknown"}`,
    `Correction maintains deliverable: ${typeof summary.session?.workflowContinuity?.coding?.correctionMaintainsDeliverable === "boolean" ? String(summary.session.workflowContinuity.coding.correctionMaintainsDeliverable) : "unknown"}`,
    `Deliverable changed during correction or escalation: ${typeof summary.session?.workflowContinuity?.coding?.deliverableChangedDuringCorrectionOrEscalation === "boolean" ? String(summary.session.workflowContinuity.coding.deliverableChangedDuringCorrectionOrEscalation) : "unknown"}`,
    `Deliverable accepted: ${typeof summary.session?.workflowContinuity?.coding?.deliverableAccepted === "boolean" ? String(summary.session.workflowContinuity.coding.deliverableAccepted) : "unknown"}`,
    `Acceptance confidence: ${summary.session?.workflowContinuity?.coding?.acceptanceConfidence ?? "unknown"}`,
    `Completion state: ${summary.session?.workflowContinuity?.coding?.completionState ?? "unknown"}`,
    `Operator confirmation required: ${typeof summary.session?.workflowContinuity?.coding?.operatorConfirmationRequired === "boolean" ? String(summary.session.workflowContinuity.coding.operatorConfirmationRequired) : "unknown"}`,
    `Loop should terminate: ${typeof summary.session?.workflowContinuity?.coding?.shouldTerminateLoop === "boolean" ? String(summary.session.workflowContinuity.coding.shouldTerminateLoop) : "unknown"}`,
    `Acceptance reason: ${summary.session?.workflowContinuity?.coding?.acceptanceReason ?? "unknown"}`,
    `Acceptance summary: ${summary.session?.workflowContinuity?.coding?.acceptanceSummary ?? "unknown"}`,
    `Output linked to deliverable: ${typeof summary.session?.workflowContinuity?.coding?.outputLinkedToDeliverable === "boolean" ? String(summary.session.workflowContinuity.coding.outputLinkedToDeliverable) : "unknown"}`,
    `Files affected: ${Array.isArray(summary.session?.workflowContinuity?.coding?.outputArtifacts) && summary.session.workflowContinuity.coding.outputArtifacts.length > 0 ? [...new Set(summary.session.workflowContinuity.coding.outputArtifacts.map((artifact) => artifact.filePath))].join(", ") : "unknown"}`,
    `Last output summary: ${summary.session?.workflowContinuity?.coding?.lastOutputSummary ?? "unknown"}`,
    `Output artifacts: ${Array.isArray(summary.session?.workflowContinuity?.coding?.outputArtifacts) && summary.session.workflowContinuity.coding.outputArtifacts.length > 0 ? summary.session.workflowContinuity.coding.outputArtifacts.map((artifact) => `step ${artifact.stepIndex}: ${artifact.filePath}${artifact.diffLikeSummary ? ` | diff=${artifact.diffLikeSummary}` : ""}${artifact.linkedToDeliverable ? " | linked=true" : " | linked=false"}`).join(" ; ") : "unknown"}`,
    `Approval state: ${summary.session?.workflowContinuity?.coding?.approvalStateSummary ?? "unknown"}`,
    `Repo action summary: ${summary.session?.workflowContinuity?.coding?.repoActionSummary ?? "unknown"}`,
    `Readiness guardrails: ${summary.session?.workflowContinuity?.coding?.integritySummary ?? "unknown"}`,
    `Pending repo actions: ${Array.isArray(summary.session?.workflowContinuity?.coding?.pendingRepoActions) && summary.session.workflowContinuity.coding.pendingRepoActions.length > 0 ? summary.session.workflowContinuity.coding.pendingRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "unknown"}`,
    `Approved repo actions: ${Array.isArray(summary.session?.workflowContinuity?.coding?.approvedRepoActions) && summary.session.workflowContinuity.coding.approvedRepoActions.length > 0 ? summary.session.workflowContinuity.coding.approvedRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "unknown"}`,
    `Executed repo actions: ${Array.isArray(summary.session?.workflowContinuity?.coding?.executedRepoActions) && summary.session.workflowContinuity.coding.executedRepoActions.length > 0 ? summary.session.workflowContinuity.coding.executedRepoActions.map((action) => `${action.actionId}: ${action.artifactFilePaths.join(", ")} | status=${action.executionStatus}`).join(" ; ") : "unknown"}`,
    `Current validation target: ${summary.session?.workflowContinuity?.coding?.currentValidationTarget ?? "unknown"}`,
    ...
    `Last implementation summary: ${summary.session?.workflowContinuity?.coding?.lastImplementationSummary ?? "unknown"}`,
    `Last validation summary: ${summary.session?.workflowContinuity?.coding?.lastValidationSummary ?? "unknown"}`,
    ...
    `Last correction summary: ${summary.session?.workflowContinuity?.coding?.lastCorrectionSummary ?? "unknown"}`,
    `Validation-first coding behavior: ${typeof summary.session?.workflowContinuity?.coding?.validationFirstActive === "boolean" ? String(summary.session.workflowContinuity.coding.validationFirstActive) : "unknown"}`,
    `Repeated validation failure driving escalation: ${typeof summary.session?.workflowContinuity?.coding?.repeatedValidationFailureDrivingEscalation === "boolean" ? String(summary.session.workflowContinuity.coding.repeatedValidationFailureDrivingEscalation) : "unknown"}`,
