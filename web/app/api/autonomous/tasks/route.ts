...
export async function GET() {
  try {
    ...
    const taskSummaries = tasks.map((task) => ({
      ...
      isContinuation: task.continuationGeneration > 0,
      ...
      continuation: {
        generation: task.continuationGeneration,
        sourceNodeId: task.continuationSourceNodeId ?? null,
        targetNodeId: task.continuationTargetNodeId ?? null,
        reason: task.continuationReason ?? null,
        priorLeaseId: task.priorLeaseId ?? null,
        resumedFromCheckpointReference: task.resumedFromCheckpointReference ?? null,
        resumedFromContinuationToken: task.resumedFromContinuationToken ? "present" : null,
    ...
    const summary = {
      ...
      continued: tasks.filter((task) => task.continuationGeneration > 0).length,
