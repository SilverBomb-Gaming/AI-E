...
import { executeSandboxUnityScriptDispatch } from "@/lib/aie/sandboxUnityScriptDispatch";

export const runtime = "nodejs";

const SAFE_ERROR_MESSAGE = "Sandbox Unity script dispatch request could not be processed.";

type SandboxUnityScriptRequestBody = {
...
function normalizeBody(src: Record<string, unknown>): SandboxUnityScriptRequestBody | null {
...
export async function POST(request: Request) {
  if (process.env.AIE_SANDBOX_UNITY_SCRIPT_ENABLED !== "true") {
    return NextResponse.json(
      {
        error: "Sandbox Unity script dispatch is not enabled in this environment.",
        hint: "Set AIE_SANDBOX_UNITY_SCRIPT_ENABLED=true to enable governed sandbox Unity script mutation.",
  ...
  try {
    const result = await executeSandboxUnityScriptDispatch({
    ...
    console.error("[api/operator/sandbox-unity-script] error", { message });
    return NextResponse.json(
