...
import { executeSandboxNotesDispatch } from "@/lib/aie/sandboxNotesDispatch";

export const runtime = "nodejs";

const SAFE_ERROR_MESSAGE = "Sandbox notes dispatch request could not be processed.";

type SandboxNotesRequestBody = {
...
function normalizeBody(src: Record<string, unknown>): SandboxNotesRequestBody | null {
...
export async function POST(request: Request) {
  if (process.env.AIE_SANDBOX_NOTES_ENABLED !== "true") {
    return NextResponse.json(
      {
        error: "Sandbox notes dispatch is not enabled in this environment.",
        hint: "Set AIE_SANDBOX_NOTES_ENABLED=true to enable real governed sandbox notes mutation.",
  ...
  try {
    const result = await executeSandboxNotesDispatch({
    ...
    console.error("[api/operator/sandbox-notes] error", { message });
    return NextResponse.json(
