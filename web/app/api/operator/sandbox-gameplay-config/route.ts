...
import { executeSandboxGameplayConfigDispatch } from "@/lib/aie/sandboxGameplayConfigDispatch";

export const runtime = "nodejs";

const SAFE_ERROR_MESSAGE = "Sandbox gameplay config dispatch request could not be processed.";

type SandboxGameplayConfigRequestBody = {
...
function normalizeBody(src: Record<string, unknown>): SandboxGameplayConfigRequestBody | null {
...
export async function POST(request: Request) {
  if (process.env.AIE_SANDBOX_GAMEPLAY_CONFIG_ENABLED !== "true") {
    return NextResponse.json(
      {
        error: "Sandbox gameplay config dispatch is not enabled in this environment.",
        hint: "Set AIE_SANDBOX_GAMEPLAY_CONFIG_ENABLED=true to enable real governed sandbox gameplay config mutation.",
  ...
  try {
    const result = await executeSandboxGameplayConfigDispatch({
    ...
    console.error("[api/operator/sandbox-gameplay-config] error", { message });
    return NextResponse.json(
