...
import { executeSandboxPatchDispatch } from "@/lib/aie/sandboxPatchDispatch";

export const runtime = "nodejs";

const SAFE_ERROR_MESSAGE = "Sandbox patch dispatch request could not be processed.";

type SandboxPatchRequestBody = {
...
function normalizeBody(src: Record<string, unknown>): SandboxPatchRequestBody | null {
...
export async function POST(request: Request) {
  if (process.env.AIE_SANDBOX_PATCH_ENABLED !== "true") {
    return NextResponse.json(
      {
        error: "Sandbox patch dispatch is not enabled in this environment.",
        hint: "Set AIE_SANDBOX_PATCH_ENABLED=true to enable real governed sandbox patch execution.",
  ...
  try {
    const result = await executeSandboxPatchDispatch({
    ...
    console.error("[api/operator/sandbox-patch] error", { message });
    return NextResponse.json(
