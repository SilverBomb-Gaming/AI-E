    ...
    <main className="min-h-screen bg-[#f6f8fb] px-6 py-8 text-slate-950">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-700">AI-E Manual System Phase 1</p>
        ...
        <section className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <nav className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-6 lg:self-start">
            <h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-slate-500">Manual Sections</h2>
            <ol className="mt-4 space-y-2">
              {docs.map((doc) => (
                <li key={doc.slug}>
                  <a href={`#${doc.slug}`} className="block rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-cyan-300 hover:bg-cyan-50">
                    {doc.title}
                  </a>
                </li>
              ))}
          ...
          <div className="space-y-6">
            {docs.map((doc) => (
              <article key={doc.slug} id={doc.slug} className="scroll-mt-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-700">{doc.fileName}</p>
                    <h2 className="mt-1 text-2xl font-semibold">{doc.heading}</h2>
                  </div>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">Markdown Source</span>
                </div>
                <pre className="mt-5 whitespace-pre-wrap break-words rounded-md border border-slate-200 bg-slate-50 p-4 text-sm leading-7 text-slate-700">
                  {doc.content}
                </pre>
              </article>
            ))}
