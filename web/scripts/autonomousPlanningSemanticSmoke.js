const { chromium } = require("playwright");

function extractSection(page, headingName) {
  return page
    .getByRole("heading", { name: headingName, exact: true })
    .locator("xpath=ancestor::section[1]")
    .innerText();
}

async function maybeExtractSection(page, headingName) {
  try {
    return await extractSection(page, headingName);
  } catch {
    return null;
  }
}

async function waitForOperatorPage(page, url, timeout = 120000) {
  const startedAt = Date.now();
  let lastError = null;

  while ((Date.now() - startedAt) < timeout) {
    try {
      const response = await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
      if (response && response.ok()) {
        return;
      }
      lastError = new Error(`Navigation reached ${url} without an OK response.`);
    } catch (error) {
      lastError = error;
    }

    await page.waitForTimeout(500);
  }

  throw new Error([
    `Timed out waiting for ${url} to become reachable.`,
    lastError instanceof Error ? lastError.message : String(lastError),
  ].join("\n\n"));
}

async function waitForSectionText(page, headingName, predicate, timeout = 20000) {
  const startedAt = Date.now();
  let lastSectionText = null;

  while ((Date.now() - startedAt) < timeout) {
    try {
      lastSectionText = await extractSection(page, headingName);
      if (predicate(lastSectionText)) {
        return lastSectionText;
      }
    } catch {
      lastSectionText = null;
    }

    await page.waitForTimeout(250);
  }

  throw new Error(`Timed out waiting for section ${headingName}. Last text: ${lastSectionText ?? "<unavailable>"}`);
}

async function snapshot(page, label) {
  return {
    label,
    stateSource: await page.locator("main").locator("text=State Source:").first().innerText(),
    planning: await extractSection(page, "Autonomous Work Planning"),
    reviewPackages: await extractSection(page, "Review Packages"),
    executionChains: await extractSection(page, "Execution Chains"),
    runtimeIntrospection: await extractSection(page, "Runtime Introspection"),
    runtimeTimeline: await extractSection(page, "Runtime Timeline"),
  };
}

async function clickApproveForWorkItem(page, title) {
  const card = page
    .getByRole("heading", { name: title, exact: true })
    .locator("xpath=ancestor::article[1]");
  await card.getByRole("button", { name: "Approve + Queue", exact: true }).click();
}

async function main() {
  const url = process.env.AIE_OPERATOR_SMOKE_URL || "http://127.0.0.1:3014/operator";
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    await waitForOperatorPage(page, url);
    await page.getByText("State Source: Live Runtime", { exact: false }).waitFor({ timeout: 30000 });
    await page.getByText("Client Controls: Ready", { exact: false }).waitFor({ timeout: 30000 });
    await waitForSectionText(
      page,
      "Autonomous Work Planning",
      (sectionText) => sectionText.includes("Deterministic planning candidate") && sectionText.includes("High-risk planning escalation"),
      30000,
    );

    const before = await snapshot(page, "before_approval");

    await clickApproveForWorkItem(page, "Deterministic planning candidate");
    await waitForSectionText(
      page,
      "Execution Chains",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /(active|completed)/i.test(sectionText),
      30000,
    );
    await waitForSectionText(
      page,
      "Runtime Timeline",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /execution chain/i.test(sectionText) && /safety gate:\s+passed/i.test(sectionText),
      30000,
    );

    const immediate = await snapshot(page, "after_approval");

    await waitForSectionText(
      page,
      "Autonomous Work Planning",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /needs review/i.test(sectionText),
      45000,
    );
    await waitForSectionText(
      page,
      "Review Packages",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /pending/i.test(sectionText),
      45000,
    );
    await waitForSectionText(
      page,
      "Execution Chains",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /completed/i.test(sectionText),
      45000,
    );
    await waitForSectionText(
      page,
      "Runtime Introspection",
      (sectionText) => /Latest safety gate decision/i.test(sectionText) && /passed/i.test(sectionText),
      45000,
    );

    const delayed = await snapshot(page, "after_completion");

    await page.reload({ waitUntil: "domcontentloaded" });
    await page.getByText("State Source: Live Runtime", { exact: false }).waitFor({ timeout: 15000 });
    await waitForSectionText(
      page,
      "Review Packages",
      (sectionText) => /planning-goal-1/i.test(sectionText) && /pending/i.test(sectionText),
      15000,
    );

    const refreshed = await snapshot(page, "after_refresh");

    console.log(JSON.stringify({ url, before, immediate, delayed, refreshed }, null, 2));
  } finally {
    const planning = await maybeExtractSection(page, "Autonomous Work Planning");
    if (planning === null) {
      // no-op diagnostics hook to keep teardown symmetric with other smoke scripts
    }
    await browser.close();
  }
}

main().catch((error) => {
  console.error(error);
  process.exit(1);
});