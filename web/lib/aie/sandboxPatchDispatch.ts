// AI-E SANDBOX PATCH DISPATCH (EXEC-0052-C)
//
// Orchestrates the first useful governed sandbox mutation:
// authorization → workspace prep → before-snapshot → adapter.invoke()
// → after-snapshot → diff → receipt → rollback contract → dispatch record.
//
// Uses sandboxPatchRuntimeAdapter: reads sandboxPatch.ts, increments patchVersion,
// writes result. On first invocation: creates the file with patchVersion = 1.
//
...
export type SandboxPatchDispatchRequest = BuildGovernedSandboxScaffoldInput & {
...
export type SandboxPatchDispatchResult = {
...
function makePatchDispatchId(now: () => string): string {
  const ts = now().replace(/[^0-9]/g, "").slice(0, 14) || "00000000000000";
  return `dispatch-${ts}-exec0052c`;
}

function withPatchTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error(`Sandbox patch dispatch timed out after ${ms}ms`)),
      ms,
    );
    promise.then(
...
function buildPatchRollbackContract(
  sandboxId: string,
  diffEntries: GovernedDispatchDiffEntry[],
  outcome: "completed" | "failed" | "timeout" | "rejected",
  nowStr: string,
...
function makePatchAuthDeniedResult(params: {
  dispatchId: string;
  invocationId: string;
  ...
  const rollbackContract = buildPatchRollbackContract(
  ...
  return {
    ...
    adapterVersion: params.adapterVersion,
    operationRequest: params.operationRequest,
    patchFilePath: SANDBOX_PATCH_FILE_NAME,
    receiptId: "",
    receiptSandboxPath: "",
...
export async function executeSandboxPatchDispatch(
  request: SandboxPatchDispatchRequest,
): Promise<SandboxPatchDispatchResult> {
  ...
  const operationRequest = request.operationRequest?.trim() || "Apply sandbox patch mutation";
  const operatorId = request.operatorId?.trim() || "operator";

  if (request.approvalToken !== GOVERNED_DISPATCH_APPROVAL_TOKEN) {
    throw new Error(
      `Sandbox patch dispatch rejected: approvalToken must be "${GOVERNED_DISPATCH_APPROVAL_TOKEN}".`,
    );
  }
  ...
  const dispatchId = makePatchDispatchId(stableNow);
  ...
  if (!authReceipt.result.authorized) {
    ...
    return makePatchAuthDeniedResult({
      dispatchId,
      invocationId,
      scaffold,
      operationRequest,
  ...
  // Resolve adapter — use injected adapter, or default sandbox patch adapter
  const adapterFs: SandboxPatchAdapterFilesystem = {
    createDirectory: resolvedFs.createDirectory,
    writeFile: resolvedFs.writeFile,
    readFile: async (p) => {
      try {
  ...
  const adapter: GovernedRuntimeAdapter = request.adapter ?? createSandboxPatchRuntimeAdapter(adapterFs);
  const adapterId = adapter.adapterId;
  const adapterVersion = adapter.adapterVersion;

  const patchFilePath = path.join(scaffold.workspace.absolutePath, SANDBOX_PATCH_FILE_NAME);
  ...
  const makeFailureResult = (
    ...
    const emptyDiff: GovernedDispatchDiffEntry[] = [];
    const rc = buildPatchRollbackContract(scaffold.sandboxId, emptyDiff, outcome, nowStr, errMsg);
  ...
  try {
    ...
    try {
      invResult = await withPatchTimeout(adapter.invoke(invocation), timeoutMs);
    } catch (timeoutError) {
    ...
    // Diff
    const diffEntries = computePatchDiff(
    ...
    const receiptRecord = await createSandboxExecutionReceipt({
      ...
      dryRun: false,
      writeReceipt: true,
      affectedFiles: [{ path: SANDBOX_PATCH_FILE_NAME, changeKind: "create", requiredApproval: true }],
      snapshotReferences: [
      ...
      plannedActions: [{
        actionId: `${dispatchId}-patch`,
        title: `Patch ${SANDBOX_PATCH_FILE_NAME} in sandbox workspace`,
        description: `Read and increment patchVersion in ${SANDBOX_PATCH_FILE_NAME} via ${adapterId}.`,
        category: "write",
