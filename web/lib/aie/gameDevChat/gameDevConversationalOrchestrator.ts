...
const capabilityPattern = /^(help\??|what\s+can\s+you\s+do\??|what\s+do\s+you\s+do\??|how\s+can\s+you\s+help\??|what\s+are\s+you\s+for\??|show\s+me\s+your\s+capabilities\.?|capabilities\??|can\s+you\s+show\s+me\s+around\??|show\s+me\s+around\??|give\s+me\s+a\s+tour\??|help\s+me\s+(learn|start|understand)\s+ai-e\??|i'?m\s+new\s+to\s+this\.?.*)$/i;
