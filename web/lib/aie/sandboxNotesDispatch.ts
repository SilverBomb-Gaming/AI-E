// AI-E SANDBOX NOTES DISPATCH (EXEC-0052-C)
//
// Orchestrates the first useful governed markdown mutation:
// authorization → workspace prep → before-snapshot → adapter.invoke()
// → after-snapshot → diff → receipt → rollback contract → dispatch record.
//
// Uses sandboxNotesRuntimeAdapter: reads sandboxNotes.md, appends one bullet note.
// On first invocation: creates the file with a header then appends.
//
...
export type SandboxNotesDispatchRequest = BuildGovernedSandboxScaffoldInput & {
...
export type SandboxNotesDispatchResult = {
  manifestVersion: "EXEC-0052-C";
...
function makeNotesDispatchId(now: () => string): string {
  const ts = now().replace(/[^0-9]/g, "").slice(0, 14) || "00000000000000";
  return `dispatch-${ts}-exec0052c-notes`;
}

function withNotesTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error(`Sandbox notes dispatch timed out after ${ms}ms`)),
      ms,
    );
    promise.then(
...
function buildNotesRollbackContract(
  sandboxId: string,
  diffEntries: GovernedDispatchDiffEntry[],
  outcome: "completed" | "failed" | "timeout" | "rejected",
  nowStr: string,
...
function makeNotesAuthDeniedResult(params: {
  dispatchId: string;
  invocationId: string;
  ...
  const rollbackContract = buildNotesRollbackContract(
  ...
  return {
    ...
    adapterVersion: params.adapterVersion,
    operationRequest: params.operationRequest,
    notesFilePath: SANDBOX_NOTES_FILE_NAME,
    receiptId: "",
    receiptSandboxPath: "",
...
export async function executeSandboxNotesDispatch(
  request: SandboxNotesDispatchRequest,
): Promise<SandboxNotesDispatchResult> {
  ...
  const operationRequest = request.operationRequest?.trim() || "Added governed stamina cooldown note";
  const operatorId = request.operatorId?.trim() || "operator";

  if (request.approvalToken !== GOVERNED_DISPATCH_APPROVAL_TOKEN) {
    throw new Error(
      `Sandbox notes dispatch rejected: approvalToken must be "${GOVERNED_DISPATCH_APPROVAL_TOKEN}".`,
    );
  }
  ...
  const dispatchId = makeNotesDispatchId(stableNow);
  ...
  if (!authReceipt.result.authorized) {
    ...
    return makeNotesAuthDeniedResult({
      dispatchId,
      invocationId,
      scaffold,
      operationRequest,
  ...
  // Resolve adapter — use injected or default notes adapter
  const adapterFs: SandboxNotesAdapterFilesystem = {
    createDirectory: resolvedFs.createDirectory,
    writeFile: resolvedFs.writeFile,
    readFile: async (p) => {
      try {
  ...
  const adapter: GovernedRuntimeAdapter = request.adapter ?? createSandboxNotesRuntimeAdapter(adapterFs);
  const adapterId = adapter.adapterId;
  const adapterVersion = adapter.adapterVersion;

  const notesFilePath = path.join(scaffold.workspace.absolutePath, SANDBOX_NOTES_FILE_NAME);
  ...
  const makeFailureResult = (
    ...
    const emptyDiff: GovernedDispatchDiffEntry[] = [];
    const rc = buildNotesRollbackContract(scaffold.sandboxId, emptyDiff, outcome, nowStr, errMsg);
    ...
    return {
      ...
      operationRequest, notesFilePath,
  ...
  try {
    ...
    try {
      invResult = await withNotesTimeout(adapter.invoke(invocation), timeoutMs);
    } catch (timeoutError) {
    ...
    const diffEntries = computeNotesDiff(
    ...
    const receiptRecord = await createSandboxExecutionReceipt({
      ...
      affectedFiles: [{ path: SANDBOX_NOTES_FILE_NAME, changeKind: "create", requiredApproval: true }],
      snapshotReferences: [
      ...
      plannedActions: [{
        actionId: `${dispatchId}-append-note`,
        title: `Append note to ${SANDBOX_NOTES_FILE_NAME}`,
        description: `Append one bullet note to ${SANDBOX_NOTES_FILE_NAME} via ${adapterId}.`,
        category: "write",
