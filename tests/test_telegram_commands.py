...
class TelegramCommandTests(unittest.TestCase):
    ...
    def test_help_command_formatting_is_concise_and_lists_commands(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("/eval", lines[7])
            self.assertIn("/chain", lines[7])
            self.assertIn("/data", lines[8])
            self.assertIn("/capabilities", lines[8])
            self.assertIn("/contexts", lines[8])
            self.assertIn("/ask", lines[9])
            self.assertIn("askctx", lines[9])
            self.assertIn("/explainrepo", lines[10])
            self.assertIn("summarizeweb", lines[10])
            self.assertIn("/workflows", lines[11])
            self.assertIn("cancelworkflow", lines[11])
