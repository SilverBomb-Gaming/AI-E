...
def _run_powershell_step(
    ...
    *,
    ...
    return run_monitored_powershell_step(
        name=name,
...
def run_monitored_powershell_step(
    *,
    name: str,
    script_path: Path,
    arguments: List[str],
    ...
    stdout_path = stdout_path or (logs_dir / f"{slug}.out.log")
    stderr_path = stderr_path or (logs_dir / f"{slug}.err.log")
    ensure_dir(stdout_path.parent)
    ensure_dir(stderr_path.parent)
    command = [* _POWERSHELL_PREFIX, "-File", str(script_path), *arguments]
    started = time.time()
    requested_stop = stop_requested or (lambda: False)
    requested_reason = stop_reason or (lambda: None)
    ...
    if interrupted:
        command_result["interrupted"] = True
        command_result["interrupt_reason"] = requested_reason() or "operator_interrupt"
    if timed_out:
