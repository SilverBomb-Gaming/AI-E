...
class RuntimeStateSnapshot:
    ...
    tasks_executed_count: int
    loop_iterations: int
    last_task_result_status: str | None
    ...
    rating_locked: bool
    last_selected_task_id: str | None
    last_selection_score: int | None
    last_selection_reason: str | None
    ...
    def to_payload(self) -> Dict[str, Any]:
        return {
            ...
            "tasks_executed_count": self.tasks_executed_count,
            "loop_iterations": self.loop_iterations,
            "last_task_result_status": self.last_task_result_status,
            ...
            "last_selection_score": self.last_selection_score,
            "last_selection_reason": self.last_selection_reason,
            "top_candidates": [dict(candidate) for candidate in self.top_candidates],
            "excluded_tasks": [dict(entry) for entry in self.excluded_tasks],
...
class RuntimeState:
    ...
    def get_snapshot(self) -> RuntimeStateSnapshot:
        ...
        return RuntimeStateSnapshot(
            ...
            tasks_failed=tasks_failed,
            tasks_executed_count=int(state.get("tasks_executed_count", 0) or 0),
            loop_iterations=int(state.get("loop_iterations", 0) or 0),
            last_task_result_status=state.get("last_task_result_status"),
            failure_streak_count=int(state.get("failure_streak_count", 0) or 0),
            integrity_issues=[dict(item) for item in state.get("integrity_issues", []) if isinstance(item, Mapping)],
            ...
            last_selection_score=int(state["last_selection_score"]) if state.get("last_selection_score") is not None else None,
            last_selection_reason=state.get("last_selection_reason"),
            top_candidates=[dict(item) for item in state.get("top_candidates", []) if isinstance(item, Mapping)],
            excluded_tasks=[dict(item) for item in state.get("excluded_tasks", []) if isinstance(item, Mapping)],
            selection_timestamp=state.get("selection_timestamp"),
        )
...
def selection_visibility_lines(snapshot: RuntimeStateSnapshot) -> List[str]:
    lines = [f"Last Selected Task: {snapshot.last_selected_task_id or 'none'}"]
    if snapshot.last_selection_score is not None:
        lines.append(f"Last Selection Score: {snapshot.last_selection_score}")
    if snapshot.last_selection_reason:
        lines.append(f"Selection Reason: {snapshot.last_selection_reason}")
    if snapshot.selection_timestamp:
        lines.append(f"Selection Timestamp: {snapshot.selection_timestamp}")
    lines.append(f"Top Candidates: {_format_top_candidates(snapshot.top_candidates)}")
    lines.append(f"Excluded Tasks: {_format_excluded_tasks(snapshot.excluded_tasks)}")
    return lines


def loop_visibility_lines(snapshot: RuntimeStateSnapshot) -> List[str]:
    return [
        f"Loop Iterations: {snapshot.loop_iterations}",
        f"Tasks Executed Count: {snapshot.tasks_executed_count}",
        f"Last Task Result Status: {snapshot.last_task_result_status or 'none'}",
        f"Failure Streak Count: {snapshot.failure_streak_count}",
    ]


def integrity_issue_lines(snapshot: RuntimeStateSnapshot) -> List[str]:
    if not snapshot.integrity_issues:
        return []
    lines = ["Integrity Issues:"]
    for entry in snapshot.integrity_issues:
...
def loop_visibility_payload(snapshot: RuntimeStateSnapshot) -> Dict[str, Any]:
    return {
        "loop_iterations": snapshot.loop_iterations,
        "tasks_executed_count": snapshot.tasks_executed_count,
        "last_task_result_status": snapshot.last_task_result_status,
        "failure_streak_count": snapshot.failure_streak_count,
    }


def selector_visibility_payload(snapshot: RuntimeStateSnapshot) -> Dict[str, Any]:
    return {
        "selected_task": snapshot.last_selected_task_id,
        "score": snapshot.last_selection_score,
        "reason": snapshot.last_selection_reason,
        "timestamp": snapshot.selection_timestamp,
        "top_candidates": [dict(candidate) for candidate in snapshot.top_candidates[:3]],
        "excluded_tasks": [dict(entry) for entry in snapshot.excluded_tasks],
    }
...
def _format_top_candidates(candidates: List[Dict[str, Any]]) -> str:
    ...
    for index, candidate in enumerate(candidates[:3], start=1):
        task_id = str(candidate.get("task_id") or "unknown_task")
        ...
        if isinstance(breakdown, Mapping) and breakdown:
            ...
            formatted.append(f"{index}. {task_id} (score={score}; {detail})")
        else:
            formatted.append(f"{index}. {task_id} (score={score})")
    return " | ".join(formatted)


def _format_excluded_tasks(excluded_tasks: List[Dict[str, Any]]) -> str:
    if not excluded_tasks:
        return "none"
    return "; ".join(
        f"{entry.get('task_id', 'unknown_task')} -> {entry.get('exclusion_reason') or entry.get('reason') or 'unknown'}"
        for entry in excluded_tasks[:5]
    )
...
__all__ = [
    ...
    "integrity_issue_lines",
    "loop_visibility_lines",
    "loop_visibility_payload",
    "selection_visibility_lines",
    "selector_visibility_payload",
]