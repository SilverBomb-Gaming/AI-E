...
def test_control_command_last_artifact_shows_selection_visibility(tmp_path):
    config = _make_config(tmp_path / "selection_visibility")
    session_id = "selection-visibility-session"
        ...
        json.dumps(
            {
                ...
                "tasks_completed": [],
                "tasks_failed": [],
                "artifacts_generated": ["artifacts/selector_01_TASK_VISIBLE.txt"],
                "blockers_detected": [],
    ...
    handler = _make_handler(config, session_id=session_id)

    result = handler.execute("show last artifact")

    assert result.title == "AI-E LAST ARTIFACT"
    assert "Last Selected Task: TASK_VISIBLE" in result.body
    assert "Selection Reason: Highest readiness + policy-safe + low execution cost" in result.body
    assert "Top Candidates: 1. TASK_VISIBLE (score=14; priority_weight=4, readiness=1, policy_score=2, recency_boost=2, execution_cost=0)" in result.body
    assert "Excluded Tasks: TASK_BLOCKED -> content_policy_blocked" in result.body
