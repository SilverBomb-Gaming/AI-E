...
def test_runtime_text_surfaces_append_timestamp_field():
    ...
    response_lines = [line for line in response.to_text().splitlines() if line.strip()]
    command_lines = [line for line in command.to_text().splitlines() if line.strip()]

    assert response_lines[-2] == "TIMESTAMP"
    assert is_standard_timestamp(response_lines[-1]) is True
    assert command_lines[-2] == "TIMESTAMP"
    assert is_standard_timestamp(command_lines[-1]) is True
