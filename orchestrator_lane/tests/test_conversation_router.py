...
def test_conversation_router_answers_status_queries(tmp_path):
        ...
        json.dumps(
            {
                ...
                "blockers_detected": [],
                "stop_reason": None,
                "tasks_executed_count": 3,
                "loop_iterations": 5,
                "last_task_result_status": "completed",
                "failure_streak_count": 1,
                "integrity_issues": [
                    {
                        "issue": "missing_operator_report_md",
                        "details": ["artifact_missing:operator_report.md"],
                        "recovery": "Regenerate the operator report so prior closeout remains auditable.",
                    }
                ],
                ...
                "waiting_reason": None,
                "blocked_reason": None,
                "last_selected_task_id": "LEVEL_0001_STAB",
                ...
                "selection_timestamp": "2026-03-16 23:00:09 -04:00 (Eastern Time — New York)",
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    assert "Last Selected Task: LEVEL_0001_STAB" in status_response.answer
    assert "Selection Reason: Highest readiness + policy-safe + low execution cost" in status_response.answer
    assert "Loop Iterations: 5" in status_response.answer
    assert "Tasks Executed Count: 3" in status_response.answer
    assert "Last Task Result Status: completed" in status_response.answer
    assert "Failure Streak Count: 1" in status_response.answer
    assert "Integrity Issues:" in status_response.answer
    assert "missing_operator_report_md" in status_response.answer
    assert "artifact_missing:operator_report.md" in status_response.answer
    assert "Top Candidates: 1. LEVEL_0001_STAB (score=14; priority_weight=5, readiness=1, policy_score=2, recency_boost=2, execution_cost=0) | 2. FOLLOWUP_001 (score=11; priority_weight=4, readiness=1, policy_score=2, recency_boost=1, execution_cost=0)" in status_response.answer
    assert "Excluded Tasks: BLOCKED_001 -> content_policy_blocked" in status_response.answer
    ...
    status_lines = [line for line in status_response.to_text().splitlines() if line.strip()]
    assert status_lines[-2] == "TIMESTAMP"
    assert status_lines[-1].endswith("(Eastern Time — New York)")
