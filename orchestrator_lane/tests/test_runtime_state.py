...
def test_runtime_state_returns_expected_fields_and_writes_snapshot(tmp_path):
    ...
    assert snapshot.tasks_executed_count == 1
    assert snapshot.loop_iterations >= 1
    assert snapshot.last_task_result_status == "completed"
    assert snapshot.failure_streak_count == 0
    ...
    assert snapshot.last_selected_task_id == "STATE_001"
    assert snapshot.last_selection_score is not None
    assert snapshot.last_selection_reason is not None
    assert snapshot.top_candidates[0]["task_id"] == "STATE_001"
    assert snapshot.selection_timestamp is not None
    ...
    assert payload["tasks_executed_count"] == 1
    assert payload["loop_iterations"] >= 1
    assert payload["last_task_result_status"] == "completed"
    assert payload["failure_streak_count"] == 0
    ...
    assert payload["last_selected_task_id"] == "STATE_001"
    assert payload["last_selection_score"] is not None
    assert payload["top_candidates"][0]["task_id"] == "STATE_001"
...
def test_runtime_state_exposes_selection_visibility_fields_from_state(tmp_path):
    config = _make_config(tmp_path / "runtime_state_selection_visibility")
    session_dir = config.runs_dir / "runtime-state-selection-session"
    session_dir.mkdir(parents=True, exist_ok=True)
    (session_dir / "session_state.json").write_text(
        json.dumps(
            {
                "session_id": "runtime-state-selection-session",
                "status": "running",
                "started_at": "2026-03-18 18:00:00 -04:00 (Eastern Time — New York)",
                ...
                "artifacts_generated": [],
                "blockers_detected": [],
                "last_selected_task_id": "TASK_VISIBLE",
                "last_selection_score": 14,
                "last_selection_reason": "Highest readiness + policy-safe + low execution cost",
                "top_candidates": [
                    {
                ...
                "excluded_tasks": [
                    {
                        ...
                        "exclusion_reason": "content_policy_blocked"
                    }
                ],
                "selection_timestamp": "2026-03-18 18:01:00 -04:00 (Eastern Time — New York)"
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    snapshot = RuntimeState(config, "runtime-state-selection-session").get_snapshot()

    assert snapshot.last_selected_task_id == "TASK_VISIBLE"
    assert snapshot.last_selection_score == 14
    assert snapshot.last_selection_reason == "Highest readiness + policy-safe + low execution cost"
    assert snapshot.top_candidates[0]["task_id"] == "TASK_VISIBLE"
    assert snapshot.excluded_tasks[0]["task_id"] == "TASK_BLOCKED"
    assert snapshot.selection_timestamp == "2026-03-18 18:01:00 -04:00 (Eastern Time — New York)"
