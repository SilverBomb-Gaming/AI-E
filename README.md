...
- remaining unreal capabilities stay explicitly out of scope: no unbounded looping, no recursion, no broad file-target expansion, no broad rewrites, no automatic rollback execution proof for this loop, no live proof for the `configuration_adjustment` lane yet, and no direct chat-triggered Unity mutation path
